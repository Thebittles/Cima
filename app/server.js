...
app.get("/dashboard", isLoggedIn, async (req, res) => {
  ...
  try {
    var doctorData30 = await DoctorModel.find({
  ...
  try {
    var treatmentData30 = await TreatmentModel.find({
      postedBy: req.user._id,
      created: { $gte: `${thirtyDays}` },
    }).exec();
  } catch (err) {
  ...
  SymptomModel.find({
    postedBy: req.user._id,
    symptomDate: { $gte: `${thirtyDays}T00:00:00.000+00:00` },
  })
    .then((symptomData30) => {
      ...
      const location = Object.keys(freq).find(
        (location) => freq[location] === maxFreq
      );
      ...
      const type = Object.keys(typeFreq).find(
        (type) => typeFreq[type] === maxType
      );
...
app.get("/week", isLoggedIn, urlencodedParser, async (req, res) => {
  ...
  try {
    var doctorDataWeek = await DoctorModel.find({
      postedBy: req.user._id,
      created: { $gte: `${week}` },
    }).exec();
  } catch (error) {
  ...
  try {
    var treatmentDataWeek = await TreatmentModel.find({
      postedBy: req.user._id,
      created: { $gte: `${week}` },
    }).exec();
  } catch (error) {
  ...
  SymptomModel.find({
    postedBy: req.user._id,
    symptomDate: { $gte: `${week}T00:00:00.000+00:00` },
  })
...
app.get("/year", isLoggedIn, urlencodedParser, async (req, res) => {
  ...
  try {
    var doctorDataYear = await DoctorModel.find({
      postedBy: req.user._id,
      created: { $gte: `${year}` },
    });
  } catch (error) {
  ...
  try {
    var treatmentDataYear = await TreatmentModel.find({
      postedBy: req.user._id,
      created: { $gte: `${year}` },
    });
  } catch (error) {
  ...
  SymptomModel.find({
    postedBy: req.user._id,
    symptomDate: { $gte: `${year}T00:00:00.000+00:00` },
  })
...
app.delete("/dashboard/symptom/:id", (req, res) => {
  let requestedToDoId = req.params.id;

  SymptomModel.findOneAndDelete({ _id: requestedToDoId })
    .then((result) => {
      res.redirect(200, "/dashboard");
      console.log(`Deleted symptom with ID of ${requestedToDoId}`);
    })
